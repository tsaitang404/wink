---
name: wink
description: wink wink 光学文件传输项目——终端/屏幕对摄像头传文件（喷泉码 QR，零依赖）。构建、测试、跨语言一致性、踩坑都在这里。
version: 1.0.0
---

# wink 项目技能

wink 把文件编码成喷泉码（LT）QR 帧流，屏幕/终端对镜头 wink 传文件，无网络路径。发送端两种（Rust 静态二进制 + 单 HTML），接收端一个网页。
