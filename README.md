...
- [ ] 多码：zxing 支持同时识别多个二维码，可同时渲染多个码提高带宽
- [x] ~~彩码：通过彩色二维码的 RGB 分量复用一个码，提高带宽~~（不可行，白平衡+通道串扰，#9）
...
./wink-linux-x86_64 文件 [--fps 30] [--block N] [-v<N>]
# → 显示元信息 QR（接收端扫码预览）
# → 按空格开始喷泉帧流
# 示例：./wink-linux-x86_64 文件 -v40 --fps 60   # 最大码 + 高帧率
```
...
| 参数 | 说明 | 默认 |
|---|---|---|
| `--fps <1-60>` | 帧率，越高带宽越大但接收端解码压力越大 | `30` |
| `--block <N>` | 块长字节（每个数据块大小）。不传则按二维码容量自动优化（64-512） | 自动 |
| `-v<N>` | 二维码版本 1-40（如 `-v15`/`-v20`/`-v40`）。版本越大单码容量越高，但终端渲染越宽、接收端对焦要求越高 | 自动选最大可容纳 |

> 提示：发送端支持 `wink --help` 查看内置帮助。
