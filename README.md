...
**屏幕对镜头 wink 传输文件。** 发送端把文件编码成无限流动的喷泉码 QR 流，接收端用摄像头读取重建文件。设备之间**没有任何网络路径**——payload 以光的形式传播。

## 为什么叫 wink

发送端的屏幕/终端对接收端的摄像头 wink，每一眨都是一帧 QR 码；接收端"看到"足够多帧后，文件就完整到达。一 wink，文件就到了。
